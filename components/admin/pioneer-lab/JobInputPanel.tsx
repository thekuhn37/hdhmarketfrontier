...
import { Link2, FileText, Upload, PenLine, Search, Loader2, AlertTriangle } from 'lucide-react'
...
export default function JobInputPanel({ onJobsExtracted, loading }: Props) {
  ...
  async function extractOne(rawText: string, sourceUrl?: string, platform?: string) {
    const res = await fetch('/api/pioneer-lab/extract', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ rawText, sourceUrl, sourcePlatform: platform ?? sourcePlatform, keywords }),
    })
  ...
  async function handlePaste() {
    ...
    setStatus('Extracting job data with AI...')
    try {
      const job = await extractOne(pasteText)
      await save([job])
      setPasteText('')
      setStatus('')
    } catch { setStatus('Extraction failed. Try again or use Manual Add.') }
  }

  async function handleUrls() {
    const urls = urlsText.split('\n').map(u => u.trim()).filter(u => u.startsWith('http'))
    if (!urls.length) { setStatus('Please enter at least one valid URL.'); return }
    setStatus(`Processing ${urls.length} URL(s)...`)
    ...
    for (const url of urls) {
      try {
        ...
        const fetchData = await fetchRes.json() as { blocked?: boolean; message?: string; content?: string }
        if (fetchData.blocked) {
          setStatus(`⚠ ${url}: ${fetchData.message}`)
          continue
        }
        const job = await extractOne(fetchData.content ?? '', url)
        results.push({ ...job, source_link: url })
      } catch (e) {
        setStatus(`Failed to process ${url}: ${e instanceof Error ? e.message : 'Error'}`)
      }
    }
    if (results.length) { await save(results); setUrlsText('') }
    setStatus(results.length ? '' : 'No jobs could be extracted. Use Paste Text for blocked sources.')
  }
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setStatus('')
    } catch { setStatus('File extraction failed.') }
  }
  ...
  const setManual = (key: keyof PioneerJobPost) => (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) =>
  ...
  return (
    <div className="bg-white dark:bg-[#0F172A] rounded-2xl border border-[#E5E7EB] dark:border-white/10 overflow-hidden">
      {/* Keyword input */}
      <div className="px-4 pt-4 pb-3 border-b border-[#F1F5F9] dark:border-white/5">
        <div className="flex gap-2 items-center">
          <Search size={14} className="text-[#6B7280] flex-shrink-0" />
          <input
            type="text"
            placeholder="Focus keywords (e.g. 'market data business development Singapore')..."
            value={keywords}
            onChange={e => setKeywords(e.target.value)}
            className={`${inputCls} text-xs`}
          />
        </div>
        <p className="text-[10px] text-[#9CA3AF] dark:text-slate-500 mt-1 ml-5">
          AI uses these keywords to prioritize relevance scoring
        </p>
      </div>
      ...
      <div className="p-4">
          ...
          <div className="space-y-3">
            <div className="flex gap-2">
              <select value={sourcePlatform} onChange={e => setSourcePlatform(e.target.value)}
                ...
                {['LinkedIn', 'FISD', 'Company Website', 'Greenhouse', 'Lever', 'Ashby', 'Workday', 'Indeed', 'Other'].map(p =>
            ...
            <textarea
              ...
              className={inputCls}
            />
            <button onClick={handlePaste} disabled={loading || !pasteText.trim()} className={btnCls}>
          ...
          <div className="space-y-3">
            <div className="flex gap-2 p-3 rounded-xl bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20">
              ...
              <p className="text-xs text-amber-700 dark:text-amber-300">
                Only use URLs you are permitted to access. LinkedIn and FISD URLs will prompt for manual paste.
                Public company career pages and ATS links (Greenhouse, Lever, Workday) work best.
              </p>
            </div>
            <textarea
              rows={6}
              placeholder="One URL per line:&#10;https://boards.greenhouse.io/company/jobs/123&#10;https://careers.company.com/jobs/456"
              value={urlsText}
              onChange={e => setUrlsText(e.target.value)}
              className={inputCls}
            />
            <button onClick={handleUrls} disabled={loading || !urlsText.trim()} className={btnCls}>
              {loading ? <Loader2 size={14} className="animate-spin" /> : <Search size={14} />}
              Search / Collect
            </button>
          </div>
        ...
        {tab === 'upload' && (
          <div className="space-y-3">
            <p className="text-xs text-[#6B7280] dark:text-slate-400">
              Upload a saved .txt, .html, or .csv file containing job posting content.
            </p>
            <div className="flex gap-2">
              <select value={sourcePlatform} onChange={e => setSourcePlatform(e.target.value)}
                className={`${inputCls} max-w-[160px]`}>
            ...
            <label className="flex flex-col items-center gap-3 px-6 py-8 rounded-xl border-2 border-dashed border-[#E5E7EB] dark:border-white/10 cursor-pointer hover:border-[#38BDF8] transition-colors">
              ...
              <span className="text-sm text-[#4B5563] dark:text-slate-300">
                Click to upload (.txt, .html, .csv)
              </span>
              <input
                ref={fileRef}
                type="file"
        ...
        {tab === 'manual' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              ...
              <input value={manualForm.company_name ?? ''} onChange={setManual('company_name')} className={inputCls} placeholder="e.g. Bloomberg L.P." />
            </div>
            <div>
              <label className="block text-xs font-medium text-[#6B7280] dark:text-slate-400 mb-1">Position Title *</label>
              <input value={manualForm.position_title ?? ''} onChange={setManual('position_title')} className={inputCls} placeholder="e.g. Senior Market Data Analyst" />
            </div>
            <div>
              <label className="block text-xs font-medium text-[#6B7280] dark:text-slate-400 mb-1">Company Category</label>
              <select value={manualForm.company_category ?? 'Other'} onChange={setManual('company_category')} className={inputCls}>
            ...
            <div>
              ...
              <select value={manualForm.function_type ?? 'Other'} onChange={setManual('function_type')} className={inputCls}>
            ...
            <div>
              ...
              <input value={manualForm.location ?? ''} onChange={setManual('location')} className={inputCls} placeholder="e.g. Singapore" />
            </div>
            <div>
              <label className="block text-xs font-medium text-[#6B7280] dark:text-slate-400 mb-1">Source Platform</label>
              <select value={manualForm.source_platform ?? 'Other'} onChange={setManual('source_platform')} className={inputCls}>
                {['LinkedIn', 'FISD', 'Company Website', 'Greenhouse', 'Lever', 'Ashby', 'Workday', 'Indeed', 'Other'].map(p =>
                  <option key={p} value={p}>{p}</option>)}
              </select>
            </div>
            <div className="sm:col-span-2">
              ...
              <input value={manualForm.source_link ?? ''} onChange={setManual('source_link')} className={inputCls} placeholder="https://..." />
            </div>
            <div className="sm:col-span-2">
              <label className="block text-xs font-medium text-[#6B7280] dark:text-slate-400 mb-1">Job Description</label>
              <textarea rows={4} value={manualForm.job_description ?? ''} onChange={setManual('job_description')} className={inputCls} placeholder="Brief description of the role..." />
            </div>
            <div className="sm:col-span-2">
              <button onClick={handleManual} disabled={loading} className={btnCls}>
        ...
        {status && (
          <p className="mt-3 text-xs text-[#6B7280] dark:text-slate-400 flex items-center gap-1.5">
